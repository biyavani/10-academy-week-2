...
class ReviewPreprocessor:
    ...
    def remove_duplicates(self):
        ...
        # Print a header for this step [NEW STEP]
        print("\n[NEW STEP] Removing duplicate reviews...")

        # Store the count before removing duplicates
        before_count = len(self.df)
        # Drop duplicate rows based on 'review_id' column, keeping the first occurrence
        self.df = self.df.drop_duplicates(subset=['review_id'], keep='first')
        # Calculate how many duplicate rows were removed
        removed = before_count - len(self.df)

        # If any duplicates were removed, print a message
        if removed > 0:
